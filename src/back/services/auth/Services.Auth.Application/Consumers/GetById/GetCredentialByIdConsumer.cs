    ...
    : IConsumer<GetCredentialByIdRequest>
{
    ...
    private async Task Process(ConsumeContext<GetCredentialByIdRequest> context)
    {
        ...
        BusMessageResponse response = queryResponse.IsSuccess
            ? new BusMessageResponse().Done(CredentialQueueResponse.Map(queryResponse.Value.Id, queryResponse.Value.Email, queryResponse.Value.CreatedOnUtc, queryResponse.Value.ModifiedOnUtc).Serialize())
